...
SEASTAR_THREAD_TEST_CASE(sstable_auto_scrub_and_ongiong_compaction) {
    ...
    test.run(sst_count, [&test_env] (table_for_tests& table, compaction::compaction_group_view& ts, std::vector<sstables::shared_sstable> sstables) {
        ...
        wait_on_enter("automatic_scrub_compaction_done", sst_count - compacting_sstable_count).wait();

        // Automatic scrub waited for signal twice - once before triggering auto scrub
        // and once after all eligible sstables were validated.
        wait_on_enter("automatic_scrub_wait_for_signal", signal_waits_before + 1).wait();
        ...
        cm.trigger_auto_scrub_timer();
        wait_on_enter("automatic_scrub_compaction_done", compacting_sstable_count).wait();
        
        BOOST_REQUIRE_EQUAL(table->get_sstables()->size(), sstables.size());
        for (auto& sst : *table->get_sstables()) {
            auto sst_timestamp = sst->get_automatic_scrub_timestamp();
...
// AUTO SCRUB SCRUBS WITH SCYLLA WITHOUT TIMESTAMP
// AUTO SCRUB TRIGGERED BY COMPACTION END
} // namespace
