...
void sstable::write_component_with_metadata(component_type type, scylla_metadata metadata) {
    if (!is_component_rewrite_supported(type)) {
        on_internal_error(sstlog, "Only Statistics adn Scylla components can be rewritten.");
    }
...
public:
    ...
    future<> validate_integrity() override {
        ...
        if (_type != component_type::Scylla) {
            ...
            // After receiving the Scylla component, it is now possible to verify the TOC digest.
