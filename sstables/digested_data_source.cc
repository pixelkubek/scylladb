...
class digested_file_data_source_impl : public data_source_impl {
...
public:
    digested_file_data_source_impl(input_stream<char> source, size_t len, uint32_t digest, digest_integrity_error_handler error_handler)
...
class digested_data_source : public data_source {
public:
    digested_data_source(input_stream<char> source, size_t len, uint32_t digest, digest_integrity_error_handler error_handler)
        : data_source(std::make_unique<digested_file_data_source_impl>(std::move(source), len, digest, std::move(error_handler)))
    {}
};

input_stream<char> make_digested_input_stream(input_stream<char> source, size_t len, uint32_t digest, digest_integrity_error_handler error_handler) {
    return input_stream<char>(digested_data_source{std::move(source), len, digest, std::move(error_handler)});
}
