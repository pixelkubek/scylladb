...
static future<rewrite_with_timestamp_result> maybe_scrub_sstables_validate_rewrite_with_timestamp(compaction_descriptor descriptor, compaction_group_view& table_s, compaction_sstable_replacer_fn replacer) {
    ...
    auto modifier = [] (sstables::scylla_metadata& metadata) {
        metadata.set_automatic_scrub_timestamp(db_clock::now());
    };
    ...
    co_await seastar::async( [&replacer, &result] {
...
future<compaction_result> do_scrub_sstables_validate_mode(compaction_descriptor descriptor, compaction_data& cdata, compaction_group_view& table_s, compaction_progress_monitor& progress_monitor) {
    ...
    const auto rewritten_with_timestamp = co_await maybe_scrub_sstables_validate_rewrite_with_timestamp(descriptor, table_s, std::move(descriptor.replacer));
...
future<compaction_result> scrub_sstables_validate_mode(compaction_descriptor descriptor, compaction_data& cdata, compaction_group_view& table_s, compaction_progress_monitor& progress_monitor) {
    ...
    auto res = co_await do_scrub_sstables_validate_mode(descriptor, cdata, table_s, progress_monitor);
    co_return res;
}
