...
std::string_view to_string(compaction_type_options::scrub::mode scrub_mode) {
    switch (scrub_mode) {
        ...
        case compaction_type_options::scrub::mode::validate:
            return "validate";
        case compaction_type_options::scrub::mode::handle:
            return "handle";
    }
...
private:
    ...
    private:
        void maybe_handle() {
            if (_scrub_mode == compaction_type_options::scrub::mode::handle) {
                _handler();
            }
        }
        void maybe_abort_scrub(std::function<void()> report_error) {
            ...
            if (_scrub_mode == compaction_type_options::scrub::mode::abort ||
                _scrub_mode == compaction_type_options::scrub::mode::handle) {
                report_error();
...
public:
    ...
    mutation_reader make_sstable_reader(schema_ptr s,
        ...
        return make_mutation_reader<reader>(std::move(full_scan_reader), _options.operation_mode, _validation_errors, _failed_to_fix_sstable, _options.drop_unfixable, _options.corruption_handler);
    }
