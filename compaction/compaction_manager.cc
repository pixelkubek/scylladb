...
void compaction_manager::enable() {
    ...
    auto scrub_period = _cfg.scrub_period();
    if (scrub_period > 0) {
        _scrub_period = std::chrono::seconds{scrub_period};
    }
...
bool compaction_manager::should_be_automatically_scrubbed(const sstables::shared_sstable& sst) const {
    ...
    auto last_automatic_scrub_attempt = sst->get_scrub_time();
...
future<> compaction_manager::submit_automatic_scrub(compaction_group_view& t) {
    ...
    if (sst->has_scylla_component()) {
        ...
        auto failure_callback = [] (sstables::shared_sstable sst) {
            // If the upgrade compaction fails, there might be
            // disk corruption, but the failure can also have other causes.
            // Unlike the normal rewrite compaction,
            // automatic scrub will not retry this scrub period.
            // The e
            // 
            // sst.mark_not_for_auto_scrub();
        };
    ...
    if (res->validation_errors > 0) {
        ...
        co_await on_suspected_disk_corruption();
    }
