...
future<compaction_manager::compaction_stats_opt> compaction_manager::perform_sstable_scrub_validate_mode(compaction_group_view& t, tasks::task_info info, quarantine_invalid_sstables quarantine_sstables,
    ...
    compacting_sstable_registration compacting(*this, get_compaction_state(&t));
    co_await run_with_compaction_disabled(t, [&sstables_to_compact, &compacting, &t] () -> future<> {
        // All sstables must be included.
        sstables_to_compact = co_await get_all_sstables(t);
        compacting.register_compacting(sstables_to_compact);
    }, "disabling compaction to run scrub validate");
    if (sstables_to_compact.empty()) {
        co_return compaction_stats_opt{};
    }

    co_return co_await perform_compaction<validate_sstables_compaction_task_executor>(throw_if_stopping::no, info, &t, info.id, std::move(sstables_to_compact), std::move(compacting), quarantine_sstables, may_update_timestamp);
}
...
future<> compaction_manager::submit_automatic_scrub(compaction_group_view& t) {
    ...
    auto do_automatic_scrub = [this, gh = std::move(gh)] (compaction_group_view& t, sstables::shared_sstable sst, tasks::task_info info, compacting_sstable_registration registration) -> future<compaction_manager::compaction_stats_opt> {
        if (sst->has_scylla_component()) {
